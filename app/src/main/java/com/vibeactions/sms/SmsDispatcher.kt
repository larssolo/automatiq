...
class SmsDispatcher @Inject constructor(
    ...
    suspend fun send(
        ...
        try {
            ...
            if (parts.size == 1) {
                sms.sendTextMessage(recipient, null, body, sentIntent, null)
            } else {
                // One receipt per part; any failing part flips the log entry to FAILED.
                val receipts = sentIntent?.let { ArrayList(parts.map { _ -> sentIntent }) }
                sms.sendMultipartTextMessage(recipient, null, parts, receipts, null)
            }
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
